...
export const register = async (req, res) => {
    const { name, username, email, password, currency='USD' } = req.body;
    ...
    try {
        ...
        const userResult = await client.query(
            ...
            [displayName, email, passwordHash, currency]
        );
...
export const login = async (req, res) => {
    ...
    try {
        const result = await pool.query('SELECT id, name, email, password_hash, currency, created_at FROM users WHERE email = $1', [email]);
